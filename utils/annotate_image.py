...
def annotate(frame_e, frame_d, xywh_e, xywh_d, xyxy_e, xyxy_d, disp, dist):
    annotate_position(frame_e, frame_d, xywh_e, xywh_d, xyxy_e, xyxy_d)
    annotate_dist(frame_e, xyxy_e, dist)
    annotate_disp(frame_d, xyxy_d, disp)
...
def annotate_position(frame_e, frame_d, xywh_e, xywh_d, xyxy_e, xyxy_d):
    cv.putText(frame_e, f'px:{int(xywh_e[0])}',
                ...
                cv.FONT_HERSHEY_PLAIN, 1, (0,0,0), 3)
...
def annotate_dist(frame_e, xyxy_e, dist):
    cv.putText(frame_e, f'x: {str(dist[0]):6s}',
                (int(xyxy_e[0]+5), int(xyxy_e[3]-35)),
                cv.FONT_HERSHEY_PLAIN, 1, (0,0,0), 3)
    cv.putText(frame_e, f'y: {str(dist[1]):6s}',
                (int(xyxy_e[0]+5), int(xyxy_e[3]-20)),
                cv.FONT_HERSHEY_PLAIN, 1, (0,0,0), 3)
    cv.putText(frame_e, f'z: {str(dist[2]):6s}',
...
def save(original_e, original_d, rectified_e, rectified_d, annotated_e, annotated_d):
    ...
    cv.imwrite("./imagens para tcc/anotada_e123456789.png", annotated_e)
    cv.imwrite("./imagens para tcc/anotada_d123456789.png", annotated_d)